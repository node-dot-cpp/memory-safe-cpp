...
#define EATEST_VERIFY(expression)           NODECPP_ASSERT(safememory::module_id, nodecpp::assert::AssertLevel::critical, ((expression)) );//          EA::UnitTest::TestInternal::EATEST_VERIFY_IMP((expression), nErrorCount, __FILE__, __LINE__, (#expression))
#define EATEST_VERIFY_MSG(expression, msg)  NODECPP_ASSERT(safememory::module_id, nodecpp::assert::AssertLevel::critical, ((expression)) );
...
/// Example usage:
///    EASTLTest_Rand rng;
///    size_t   x = rng();                      // Generate value in range of [0, 0xffffffff] (i.e. generate any uint32_t)
///    intptr_t  y = rng.Rand(1000);             // Generate value in range of [0, 1000)
///    intptr_t  z = rng.RandRange(-50, +30);    // Generate value in range of [-50, +30)
///
...
{
public:
	EASTLTest_Rand(size_t nSeed) // The user must supply a seed; we don't provide default values.
		: mnSeed(nSeed) { }
	...
	size_t Rand()
	{
		...
		return (size_t)mnSeed; // For size_t == uint32_t, this is a chop.
	}

	size_t operator()() // Returns a pseudorandom value in range of [0, 0xffffffffffffffff)] (i.e. generate any size_t)
		{ return Rand(); }

	size_t operator()(size_t n)  // Returns a pseudorandom value in range of [0, n)
		{ return RandLimit(n); }

	size_t RandLimit(size_t nLimit) // Returns a pseudorandom value in range of [0, nLimit)
	{
		...
		// return (size_t)((Rand() * (uint128_t)nLimit) >> 64);

		return (Rand() % nLimit); // This results in an imperfect distribution, especially for the case of nLimit being high relative to size_t.
	}

	intptr_t RandRange(intptr_t nBegin, intptr_t nEnd)   // Returns a pseudorandom value in range of [nBegin, nEnd)
		{ return nBegin + (intptr_t)RandLimit((size_t)(nEnd - nBegin)); }
