...
template struct safe_memory::array<int>;
template struct safe_memory::array<Align32>; // VC++ fails to compile due to error generated by the swap function. C2718: http://msdn.microsoft.com/en-us/library/vstudio/sxe76d9e.aspx


template<bool C, typename ARR>
int TestArrayImpl(ARR& a, ARR& b, ARR& c)
{
	...
	{ 
		...
		// rbegin, rend
		ARR::reverse_iterator itr = a.rbegin();
...
template<template<typename, size_t> typename ARR, bool C> 
int TestArrayStack()
{
...
int TestArrayHeap()
{
	...
	auto a = safe_memory::make_owning<ARR<int, 5>>(ia);
	auto b = safe_memory::make_owning<ARR<int, 5>>(ib);
	auto c = safe_memory::make_owning<ARR<int, 5>>(ic);
...
using arr1 = safe_memory::array<T, SZ, safe_memory::memory_safety::none>;

template<typename T, size_t SZ>
using arr2 = safe_memory::array<T, SZ, safe_memory::memory_safety::safe>;

template<typename T, size_t SZ>
using arr3 = safe_memory::array_safe<T, SZ, safe_memory::memory_safety::none>;

template<typename T, size_t SZ>
using arr4 = safe_memory::array_safe<T, SZ, safe_memory::memory_safety::safe>;
...
int TestArray()
{
	...
	nErrorCount += TestArrayStack<arr1, false>();
	nErrorCount += TestArrayStack<arr2, false>();
	nErrorCount += TestArrayStack<arr3, false>();
	nErrorCount += TestArrayHeap<arr1, false>();
	nErrorCount += TestArrayHeap<arr2, false>();
	nErrorCount += TestArrayHeap<arr3, false>();
	nErrorCount += TestArrayHeap<arr4, false>();
