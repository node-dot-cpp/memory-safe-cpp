...
* All rights reserved.
*
* Copied from clang/lib/Frontend/Rewrite/InclusionRewriter.cpp
*
* -------------------------------------------------------------------------------*/
...
private:
  ...
  void OutputContentUpTo(const llvm::MemoryBuffer &FromFile,
                         unsigned &WriteFrom, unsigned WriteTo,
                         StringRef EOL, int &lines,
                         bool EnsureNewline);
  void CommentOutDirective(Lexer &DirectivesLex, const Token &StartToken,
                           const llvm::MemoryBuffer &FromFile, StringRef EOL,
...
static StringRef DetectEOL(const llvm::MemoryBuffer &FromFile) {
...
void InclusionRewriter::detectMainFileEOL() {
  bool Invalid;
  const llvm::MemoryBuffer &FromFile = *SM.getBuffer(SM.getMainFileID(), &Invalid);
  assert(!Invalid);
  if (Invalid)
...
/// \p WriteTo - 1.
void InclusionRewriter::OutputContentUpTo(const llvm::MemoryBuffer &FromFile,
                                          unsigned &WriteFrom, unsigned WriteTo,
...
void InclusionRewriter::Process(FileID FileId,
                                ...
                                const DirectoryLookup *DirLookup) {
  bool Invalid;
  const llvm::MemoryBuffer &FromFile = *SM.getBuffer(FileId, &Invalid);
      ...
      if (RawToken.getIdentifierInfo() != nullptr) {
        switch (RawToken.getIdentifierInfo()->getPPKeywordID()) {
          ...
          case tok::pp_import: {
            ...
            if (const IncludedFile *Inc = FindIncludeAtLocation(Loc)) {
...
void RewriteUserIncludesInInput(Preprocessor &PP, raw_ostream *OS,
  ...
  (*OS) << "#include <safememory/detail/checker_instrument.h>\n";
